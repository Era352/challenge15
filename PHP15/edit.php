...
//handle from submission to update user data
if($_SERVER["REQUEST_METHOD"]=="POST"){
    $username = mysqli_real_escape_string($conn, $_post['Title']);
    $username = mysqli_real_escape_string($conn, $_post['Description']);
    $username = mysqli_real_escape_string($conn, $_post['Qunatity']);
    $username = mysqli_real_escape_string($conn, $_post['Price']);

    if(empty($username)||empty($email)){
        $error = "All fields are required";
    }else{
        $update_sql="UPDATE users SET title = '$Title', Description = '$Description', Qunatity = '$Qunatity', Price = '$Price' WHERE id = '$id'";
        if($conn->query($update_sql)){
            $sucess="Users updated successfully";
